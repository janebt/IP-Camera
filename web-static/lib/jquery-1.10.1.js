...
jQuery.extend({
	...
	ajax: function( url, options ) {
		...
		function done( status, nativeStatusText, responses, headers ) {
			...
			isSuccess = status >= 200 && status < 300 || status === 304 || status === 401;
...
jQuery.extend({
	/* 发送Ajax请求 */
	sendAjaxReq: function(url, data, callback, async, postDataType, utf8Encode)
	{
		if (true == $.local)
		{
			try 
			{
				return callback && callback({err_code: ENONE});
			}catch(ex){
				log(ex);
			}
		}
	...
	/* 发送DS数据 */
	sendDsReq: function(data, callback, async, method)
	{
		var url, relt = {}, result;

		if (false === $.authRltObj["authStatus"])
		{

			try
			{
				relt[ERR_CODE] = ENONE;
				typeof callback == "function" && (result = callback(relt));
	...
	query: function(data, callback, async)
	{
		...
		return $.sendDsReq(data, function(rel){
			if (g_pageLoadingShow == true)
			{
	...
	queryP: function(data, callback, async)
	{
		return $.sendDsReq(data, callback, async, "get");
	},

	modify: function(data, callback, async)
	{
		return $.sendDsReq(data, callback, async, "set");
	},

	add: function(data, callback, async)
	{
		return $.sendDsReq(data, callback, async, "add");
	},

	del: function(data, callback, async)
	{
		return $.sendDsReq(data, callback, async, "delete");
	},

	action: function(data, callback, async)
	{
		return $.sendDsReq(data, callback, async, "do");
	}
});
...
jQuery.extend({
	...
	username:"",
	rootName:"admin",
	encryptPub:"-----BEGIN PUBLIC KEY-----MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQC4D6i0oD/Ga5qb//RfSe8MrPVIrMIGecCxkcGWGj9kxxk74qQNq8XUuXoy2PczQ30BpiRHrlkbtBEPeWLpq85tfubTUjhBz1NPNvWrC88uaYVGvzNpgzZOqDC35961uPTuvdUa8vztcUQjEZy16WbmetRjURFIiWJgFCmemyYVbQIDAQAB-----END PUBLIC KEY-----",
	authRltObj: 
	{
		code: "",
		key: "",
	...
	setPluginAuthHandle: function(func){
		$.pluginAuthHandle = func;
	},
	...
	/* 设置登录本地存储 */
	setLg: function(username, pwd)
	{
		try
		{
			if ($.tmpPwdMD5)
			{
				sessionLS.setItem(LGKEYMD5STR, $.tmpPwdMD5);
			}
			sessionLS.setItem(LGKEYSTR, pwd);
			sessionLS.setItem(LGUSER, username);
			$.pwd = pwd;
			$.username = username;
			if ($.pwdMD5 != $.tmpPwdMD5)
			{
			...
			if (typeof $.pluginAuthHandle == "function")
			{
				$.pluginAuthHandle(username, pwd);
			}
		}catch(ex){};
	},
	...
	parseAuthRlt: function(authRlt)
	{
		if (typeof authRlt == "object")
		{
			...
			typeof $.pagePRHandle == "function" && $.pagePRHandle();
		}
	},
	...
	logout: function()
	{
		...
		$.action(data, function(result){
			if (ENONE == result[ERR_CODE])
			{
				...
				$.accountStatus["logoutHandle"] = true;
	...
	postDsUnAuthHandle: function(data, callback, async, result)
	{
		...
		if (EUNAUTH == result[ERR_CODE])
		{
			...
			return $.auth($.username, $.pwd, function(err_code){
				if (ENONE == err_code)
				{
					/* 续约成功后再次发送中断的请求 */
	...
	encryptPwd: function(pwd){
		...
		/* fix the bug of cyassl can't decrypt the text lack of some character */
		for (var i = 0; i < maxFailTime; i++)
		{
			encrypt = new JSEncrypt();
			encrypt.setPublicKey($.encryptPub);
			rel = encrypt.encrypt(pwd);

			if (rel[0] != false && rel[1].length % 64 == 0)
			{
				break;
			}
		}
	...
	changeDefaultPwd: function(pwd, callBack)
	{
		...
		$.tmpPwdMD5 = $.getPwdMD5($.rootName, pwd);
		var reqData = {
			method : "do",
			set_password : {
				password : pwdStr,
				username : $.rootName,
				ciphertext : $.encryptPwd(pwd)
			}
		};
		...
		$.sendAjaxReq("", reqData,
			function(result){
				...
				if (ENONE == err_code)
				{
					...
					try
					{
						typeof upSysNtpTime == "function" && upSysNtpTime();
					}
					catch(ex){}
				}
	...
	/* 登录使用/修改密码 */
	auth: function(user, pwd, callBack, asyn)
	{
		var reqData, pwdStr;
		...
		/* 用户名格式错误 */
		if (user == undefined || 0 == user.length)
		{
